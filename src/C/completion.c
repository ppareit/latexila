/* completion.c generated by valac 0.10.2, the Vala compiler
...
static void _vala_array_add4 (CompletionProviderCompletionArgument** array, int* length, int* size, const CompletionProviderCompletionArgument* value);
...
static gboolean completion_provider_real_activate_proposal (GtkSourceCompletionProvider* base, GtkSourceCompletionProposal* proposal, GtkTextIter* iter) {
	...
	self = (CompletionProvider*) base;
	g_return_val_if_fail (proposal != NULL, FALSE);
	text = g_strdup (gtk_source_completion_proposal_get_text (proposal));
	if (text == NULL) {
		_tmp0_ = TRUE;
	} else {
...
static void completion_provider_parser_start (CompletionProvider* self, GMarkupParseContext* context, const char* name, char** attr_names, int attr_names_length1, char** attr_values, int attr_values_length1, GError** error) {
	...
	switch (0) {
		default:
		{
			...
			_vala_array_add3 (&self->priv->current_arg.choices, &self->priv->current_arg.choices_length1, &self->priv->current_arg._choices_size_, (_tmp28_ = (completion_provider_completion_choice_copy (&choice, &_tmp27_), _tmp27_), &_tmp28_));
			completion_provider_completion_choice_destroy (&choice);
			break;
		}
...
static void completion_provider_parser_end (CompletionProvider* self, GMarkupParseContext* context, const char* name, GError** error) {
	...
	switch (0) {
		default:
		{
			...
			_vala_array_add4 (&self->priv->current_command.args, &self->priv->current_command.args_length1, &self->priv->current_command._args_size_, (_tmp5_ = (completion_provider_completion_argument_copy (&self->priv->current_arg, &_tmp4_), _tmp4_), &_tmp5_));
			break;
		}
	}
}
...
static void completion_provider_completion_command_copy (const CompletionProviderCompletionCommand* self, CompletionProviderCompletionCommand* dest) {
	...
	dest->args = (_tmp0_ = self->args, (_tmp0_ == NULL) ? ((gpointer) _tmp0_) : _vala_array_dup1 (_tmp0_, (*self).args_length1));
...
static void completion_provider_completion_argument_copy (const CompletionProviderCompletionArgument* self, CompletionProviderCompletionArgument* dest) {
	...
	dest->choices = (_tmp0_ = self->choices, (_tmp0_ == NULL) ? ((gpointer) _tmp0_) : _vala_array_dup2 (_tmp0_, (*self).choices_length1));
